...
- **网格卡片（P0）**：类型图标（排钻项目=网格蓝图 / 精修项目=钻面文档）+ 名称 + 元信息行（`2,341 钻 · SS10 · 09-19`，直出 summary 缓存）+ 类型徽标。**缩略图 P1**：保存时渲染 256px PNG 入库（thumbKey 机制沿素材库 P1 规划）。
...
4. `content-design` 的命名审计流程（voice/术语表）在本仓尚无落盘术语表——B.3 联动表实为首个术语注册表雏形，建议后续抽 `TERMS.md` 入仓根（排钻/精修/送排钻/送精修/项目 两级词条）。
