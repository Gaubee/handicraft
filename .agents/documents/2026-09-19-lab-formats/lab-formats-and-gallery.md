...
3. **variants {v:2} 信封退役 vs 保留会话缓冲**（= design 议题 10）。
   立场（**已被 change design §9.3 推翻，仅保留为历史 PM 议题记录**）：退役 + 一次性迁移；**实现算法以 §9.3 为准**——create-only、确定性迁移 id（内置 `ast-tpl-${presetId}` / 自建 `ast-tpl-legacy-${legacyId}`）、journal 完成集、一次性备份 key（30 天 TTL，Codex 裁定采纳），**不允许任何内容覆写**（本条原「用户编辑覆写 seed」表述作废）。保留信封 = 模板双真源（结构性失败）；「离线缓冲」论点不成立——库写入失败时字段编辑保留在内存态 + 重试（A.1.4 兜底），不需要第二持久层。
