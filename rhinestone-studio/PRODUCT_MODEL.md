...
> 版本：v4（2026-09-20，随 rename-and-expert-workbench R 轨改名：「手动编辑」模块名退役为「专家工作台」，一句话/对象树/硬规则 3/真源表措辞联动）；v3（2026-09-19，随「add-project-files」四格式家族裁决引入项目文件层；v2 随素材库引入资产层，见 git 历史；v1 见 `.agents/documents/2026-09-18-rhinestone-ia/ia-design.md` §0）
> 地位：**所有界面/功能改动的第一约束**。任何需求动手前先回答「如何进入本模型」；与本文冲突的设计一律 REJECT。

## 一句话

四模块工作台 + 一个资产层：实验室产出「贴钻候选效果图」，排钻设计把效果图调成「可生产钻位方案」，专家工作台做「钻级编排精修」，素材库是**全部资产（图片 + 项目文件）的唯一真源**。
...
实验室（真源=库模板 .gemtpl） ──handoff(assetId)──▶ 排钻设计（真源=参数：块覆写/物理/色板，存 .gemproj）
                                                       │
                                                       ├─导出 .gemdoc / 送精修（ManualEditHandoff 烘焙快照）──▶ 专家工作台（真源=EditDocument.gems，存 .gemdoc）
                                                       └─▶ SVG / BOM / PNG 导出
```
