...
<section class="grid grid-cols-1 gap-2" data-testid="variant-editor">
  <div class="flex items-center gap-2">
    ...
    <span>
      <HelpTip text="每个「模板 × 候选」都是一次独立请求（恒 n:1），并发上限 4。模板保存在素材库（.gemtpl 资产，改即存）；内置模板默认与案例图一一绑定，随请求附送一张「案例参照图」（原图+效果图自动合成的合成图），原图（目标图）单列其后。提示词由系统自动拼装：图片角色声明 + 通用贴钻规则 + 模板特化正文。" />
    </span>
  </div>
