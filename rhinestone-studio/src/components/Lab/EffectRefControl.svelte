...
<div class="grid gap-1.5" data-testid="effect-ref-control">
  ...
  {:else}
    <div
      class="border-border/80 flex flex-wrap items-center gap-2 rounded-lg border border-dashed p-2.5"
  ...
  <Dialog.Root bind:open={compareOpen}>
    <Dialog.Content class="max-w-2xl">
      <Dialog.Header>
        <Dialog.Title class="text-sm">案例图 · 原图 → 贴钻效果</Dialog.Title>
        ...
        <div class="flex flex-wrap items-center justify-center gap-3">
          {#if urls.srcUrl}
            <figure class="grid gap-1">
              <img src={urls.srcUrl} alt="案例原图" class="w-64 max-w-full rounded-lg border object-contain" draggable="false" />
          ...
          {/if}
          <figure class="grid gap-1">
            <img src={urls.resUrl} alt="贴钻效果案例" class="w-64 max-w-full rounded-lg border object-contain" draggable="false" />
            <figcaption class="text-muted-foreground text-center text-xs">贴钻效果</figcaption>
          </figure>
        </div>
      {:else}
        <p class="text-muted-foreground text-xs">案例图加载中或已失效。</p>
      {/if}
    </Dialog.Content>
  </Dialog.Root>
</div>
