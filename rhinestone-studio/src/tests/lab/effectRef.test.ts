...
describe('composeDrillPrompt：WYSIWYG 组合（六层注入退场 2026-09-21）+ 案例片段默认内容', () => {
  it('公理：三开关全关语境（无占位符正文）逐字节透传——无人设/无计数行/无任务行/无规则块/无壳/无输出行', () => {
    const prompt = composeDrillPrompt('模板特化正文', { hasCase: true, caseLayout: 'horizontal', hasReference: true })
    expect(prompt).toBe('模板特化正文')
    expect(prompt).not.toContain('你是一位专业的钻石画')
    expect(prompt).not.toContain('我上传了')
    expect(prompt).not.toContain('【任务要求】')
    expect(prompt).not.toContain('【贴钻指导规则】')
    expect(prompt).not.toContain('【模板风格补充】')
    expect(prompt).not.toContain('请输出')
  })

  it('合成横 + 参考 + 占位符：片段默认内容 = 多图介绍（复合对照变体）', () => {
  ...
  it('合成纵 + 参考：角色描述说明上下两半', () => {
    const out = autoCaseRefFragment({ hasCase: true, caseLayout: 'vertical', hasReference: true })
    expect(out).toContain('案例参照合成图：上半为未贴钻的原图，下半为其 Partial Drill（局部贴钻）成品效果图。')
    expect(out).toContain('请参照【图一 [image #1]：案例参照图】所展示的「原图 → 贴钻效果」转换风格与选区逻辑，为【图二 [image #2]：参考图】生成')
  })

  it('单张 + 参考：单图描述 + 贴钻风格措辞（无原图半，不引用「转换」）', () => {
    const out = autoCaseRefFragment({ hasCase: true, caseLayout: 'single', hasReference: true })
    expect(out).toContain('1. 【图一 [image #1]：案例参照图】：案例参照图：一张已完成的 Partial Drill（局部贴钻）效果图。')
    expect(out).toContain('2. 【图二 [image #2]：参考图】：需要你处理的目标图像。')
    expect(out).toContain('请参考【图一 [image #1]：案例参照图】所展示的贴钻风格与选区逻辑，为【图二 [image #2]：参考图】生成对应的 Partial Drill 效果图。')
    expect(out).not.toContain('「原图 → 贴钻效果」转换')
  })

  it('仅案例（无参考）：同风格完整设计效果图', () => {
    const out = autoCaseRefFragment({ hasCase: true, caseLayout: 'horizontal', hasReference: false })
    expect(out).toContain('我上传了一张图片：')
    expect(out).toContain('1. 【图一 [image #1]：案例参照图】')
    expect(out).toContain('请参考【图一 [image #1]：案例参照图】所展示的贴钻风格与选区逻辑，生成一张同风格的 Partial Drill（局部贴钻）完整设计效果图。')
  })

  it('无任何附图（纯文生图）：片段默认内容为空串（零图序说明——不虚构声明）', () => {
    expect(autoCaseRefFragment({ hasCase: false, caseLayout: 'single', hasReference: false })).toBe('')
...
describe('describeDrillImageOrder：附图序号单一真源（UI 徽标与片段默认内容共用）', () => {
  it('案例+参考：图一=案例参照图、图二=原图；与片段默认内容编号一致', () => {
    ...
    // 片段默认内容引用同一编号（防两套口径漂移）
    const fragment = autoCaseRefFragment({ hasCase: true, caseLayout: 'horizontal', hasReference: true })
    for (const e of order) expect(fragment).toContain(`【图${e.figure} [image #${e.ordinal}]：${e.figureLabel}】`)
  })
...
describe('生成请求链路：images = [案例合成图, 原图]', () => {
  it('模板案例绑定 + 用户原图：edits 端点，image 顺序 = [案例合成图, 用户原图]，prompt 新角色声明', async () => {
    ...
    // 〔WYSIWYG 2026-09-21〕promptBody 无占位符 → 请求提示词 ≡ promptBody 逐字节
    // （图序说明归案例片段默认内容——正文未放置占位符即不注入）
    expect(String(editCalls[0].get('prompt'))).toBe('base rhinestone prompt')
  ...
  it('仅案例绑定无用户参考：仍走 edits，单图 = 案例参照图，任务行「同风格完整设计」', async () => {
    ...
    // 〔WYSIWYG〕无占位符正文逐字节透传（任务行/规则块已退场——片段默认内容不进请求）
    expect(String(editCalls[0].get('prompt'))).toBe('base rhinestone prompt')
  })
  ...
  it('asset kind 合成图进请求：blob 经 getAssetBlob 取回转 File；任务快照 = caseBinding', async () => {
    ...
    // 〔WYSIWYG〕无占位符正文逐字节透传
    expect(String(editCalls[0].get('prompt'))).toBe('single bound prompt')
