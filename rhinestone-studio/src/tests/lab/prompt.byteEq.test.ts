...
describe('composeDrillPrompt 两参形态字节基线（0.2 改造前直采）', () => {
  it('case(horizontal) + reference + 模板体', () => {
    expect(
      ...
      我上传了2 张图片：
      1. 【图一：案例参照图】：案例参照合成图：左半为未贴钻的原图，右半为其 Partial Drill（局部贴钻）成品效果图。
      2. 【图二：参考图】：需要你处理的目标图像。

      【任务要求】：
      请参照【图一：案例参照图】所展示的「原图 → 贴钻效果」转换风格与选区逻辑，为【图二：参考图】生成对应的 Partial Drill 效果图。

      【贴钻指导规则】：
      1. 虚实结合（Partial Drill）：不要全图贴钻。保留【图二：参考图】的大面积背景与次要细节为原始画风/印刷效果。
      2. 选区策略：仅在【图二：参考图】的视觉焦点、核心主体（如：主体的轮廓线、羽毛/花瓣脉络、眼睛、高光区）上叠加水钻装饰。
      3. 材质与折射：贴钻区域需呈现明显的立体感、光线折射感和立体水钻（Rhinestones / Gemstones）的切面光泽。
      4. 画风一致性：未贴钻的背景区域需完全保持【图二：参考图】的原有风格、构图与配色。

      【模板风格补充】：
      模板特化正文

      请输出【图二：参考图】应用局部贴钻后的最终渲染效果图。"
    `)
  })
  ...
  it('case(vertical) + reference', () => {
    expect(composeDrillPrompt('正文', { hasCase: true, caseLayout: 'vertical', hasReference: true })).toMatchInlineSnapshot(`
      ...
      我上传了2 张图片：
      1. 【图一：案例参照图】：案例参照合成图：上半为未贴钻的原图，下半为其 Partial Drill（局部贴钻）成品效果图。
      2. 【图二：参考图】：需要你处理的目标图像。
  ...
  it('case(single) + reference', () => {
    expect(composeDrillPrompt('正文', { hasCase: true, caseLayout: 'single', hasReference: true })).toMatchInlineSnapshot(`
      ...
      我上传了2 张图片：
      1. 【图一：案例参照图】：案例参照图：一张已完成的 Partial Drill（局部贴钻）效果图。
      2. 【图二：参考图】：需要你处理的目标图像。

      【任务要求】：
      请参考【图一：案例参照图】所展示的贴钻风格与选区逻辑，为【图二：参考图】生成对应的 Partial Drill 效果图。
  ...
  it('case(horizontal) 无 reference', () => {
    expect(composeDrillPrompt('正文', { hasCase: true, caseLayout: 'horizontal', hasReference: false })).toMatchInlineSnapshot(`
      ...
      【任务要求】：
      请参考【图一：案例参照图】所展示的贴钻风格与选区逻辑，生成一张同风格的 Partial Drill（局部贴钻）完整设计效果图。
  ...
  it('无 case + reference', () => {
    expect(composeDrillPrompt('正文', { hasCase: false, caseLayout: 'single', hasReference: true })).toMatchInlineSnapshot(`
      ...
      【任务要求】：
      请为【图一：参考图】生成 Partial Drill（局部贴钻）效果图，遵循以下贴钻指导规则。

      【贴钻指导规则】：
      1. 虚实结合（Partial Drill）：不要全图贴钻。保留【图一：参考图】的大面积背景与次要细节为原始画风/印刷效果。
      2. 选区策略：仅在【图一：参考图】的视觉焦点、核心主体（如：主体的轮廓线、羽毛/花瓣脉络、眼睛、高光区）上叠加水钻装饰。
      3. 材质与折射：贴钻区域需呈现明显的立体感、光线折射感和立体水钻（Rhinestones / Gemstones）的切面光泽。
      4. 画风一致性：未贴钻的背景区域需完全保持【图一：参考图】的原有风格、构图与配色。

      【模板风格补充】：
      正文

      请输出【图一：参考图】应用局部贴钻后的最终渲染效果图。"
    `)
  })
