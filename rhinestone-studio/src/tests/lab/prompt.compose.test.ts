/**
 * composeDrillPrompt 组合接线测试（〔WYSIWYG 基线再生 2026-09-21〕六层注入退场后）。
 * - options 形态等价：drillParams 缺席时各形态输出与两参形态**逐字节相等**（≡ 模板体）；
 * - n 元序号连续性（附图序号 = 角色声明序号不变量）+ 素材声明并入案例片段默认内容 +
 *   清单行交叉引用同源；
 * - 水钻段经占位符进正文（缺占位符 = 不注入）+ 截断集成 + 主图纯净性（无蓝图词汇）。
 */
import { describe, expect, it } from 'vitest'
import { autoCaseRefFragment, composeDrillPrompt, type DrillPromptImageRoles } from '$lib/presets/effectRefs'
...
describe('options 形态等价（drillParams 缺席 → 各形态输出与两参形态逐字节相等）', () => {
  it('options 各缺席形态 === 两参形态 === 模板体（全角色矩阵 × 空/多行模板体）', () => {
...
describe('n 元扩展：素材附图角色声明并入案例片段默认内容 + 序号连续性', () => {
  it('roles.materials 显式通道：案例(一) 参考(二) 素材(三/四)——声明行进入 autoCaseRefFragment', () => {
    const roles = {
      hasCase: true,
      caseLayout: 'horizontal' as const,
      hasReference: true,
    ...
    // 片段默认内容与组装器替换同源（Dialog 所见即所发）
    expect(prompt).toContain(autoCaseRefFragment(roles))
  })

  it('无占位符 = 素材声明不进正文（开+缺占位符=不注入——WYSIWYG 语义）', () => {
    const prompt = composeDrillPrompt('正文', {
      hasCase: true,
  ...
  it('drillParams 通道（specs 物化派生）：自定义形声明并入片段 + 清单交叉引用同图号', () => {
    const roles = { hasCase: true, caseLayout: 'horizontal' as const, hasReference: true }
    ...
    // 素材图从 specs 派生（drillParams 存在 = specs 是交叉引用真源——显式 materials 忽略）
    ...
    expect(prompt).not.toContain('【案例参照图提示词】')
  })

  it('水钻段占位符替换位 = 用户书写位（无输出行/独立段位——正文原样居中）', () => {
    const withPlaceholder = composeDrillPrompt(
      '特化体内容\n【水钻参数提示词】',
      { hasCase: false, caseLayout: 'single', hasReference: true },
      { drillParams: { specs: [roundSs10], materialAssetIds: [] } },
    )
    expect(withPlaceholder.startsWith('特化体内容\n')).toBe(true)
    expect(withPlaceholder).toContain('【尺寸与钻规格】')
    expect(withPlaceholder.endsWith('\n  1 = R10 圆形 SS10（直径 2.8mm）')).toBe(true)
  ...
  it('主图纯净性：drillParams 注入不含任何蓝图词汇（蓝图要求只经蓝图占位符/蓝图 stage）', () => {
    const prompt = composeDrillPrompt(
      '正文',
      { hasCase: false, caseLayout: 'single', hasReference: true },
