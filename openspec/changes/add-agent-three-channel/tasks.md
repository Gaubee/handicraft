...
- [x] 3.2 真环境走查（rpc 模式装载会话→打断/排队/引导三通道操作截屏+程序化状态转换断言——/tmp/w10-walkthrough-{1..4}-*.png）
- [x] 3.3 三仓门禁+tasks 勾选+验收报告（聚焦测试全绿+两包 tsc+svelte-check 余 3 错均系波 2a 契约漂移归属+build 过）
