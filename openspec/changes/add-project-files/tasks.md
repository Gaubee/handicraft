...
1. [2026-09-19 Contract] projectFile.ts 两格式序列化契约先冻（round-trip 字节等价 + formatVersion 迁移链），
   引擎仅增 ENGINE_VERSION 常量与 bump 纪律注释，不改任何算法语义。
2. [2026-09-19 Data] AssetProject 入库（ingest + blobKey 换绑保存 + sys-projects + 保护④）全 vitest 证明，
   含失败注入；库内可见性回归防「项目节点隐形」。
3. [2026-09-19 UX] 两页生命周期（studio 项目态 / edit dirty+四入口）与守卫三分法；上下文条重写、
   编辑空态重写、改名联动一次改齐；不回改五区其余结构。
4. [2026-09-19 Process] §E 七议题 Codex 裁决前，对应切片不实现（议题→任务映射见各节标注）；每步绿门
   pnpm test + svelte-check + build；浏览器走查覆盖 PM 稿 §C 动线。
-->
...
- [ ] 0.4 [R1-B1] runTx 完成语义改造：body 返回值暂存 + `tx.oncomplete` resolve / `onabort/onerror` reject；vitest：换绑三态、注入 nodes.put/images.put/delete 失败旧态保持、共享 blob 引用不误删（**全部后续存储切片的 GO 前置**）
- [ ] 0.5 [R1-B5] AssetProject 生命周期 API 冻结：`openProject/closeProject`（pin 引用计数 + owner/token，最后 owner 关闭才解除）+ `updateProjectAsset`（单确认事务 CAS 换绑，expectedBlobKey 乐观锁）+ summary 非真源规则（带 summaryUpdatedAt，缺/坏可重算不覆盖）；vitest：计数解除/CAS 冲突/GC 全无引用才删
- [ ] 0.6 [R1-B2] `getHandoffImageBlob(assetId)` 单点出口（image 直取 / gemgen parse 内嵌 / missing 显式失败），studio.loadFromHandoff、下载、画廊只调此出口；HandoffPayload 零变化；vitest：三态 + 零重编码
- [ ] 0.7 [R1-B3] `openIntent.svelte.ts` 冻结：四 kind 单 store + token `peek/consumeSuccess`（exactly-once）；双击先 parse 成功再置意图；App 只切视图；LabView hydrate 就绪且动作完成才消费；vitest：五时序（未挂载/已在实验室/解析失败/模板缺失/刷新重入）
- [ ] 0.8 [R1-B10] 契约冻结 gate 收口：手势统一文本（含姊妹稿/PRODUCT_MODEL 已修文核对）+ variants 迁移 journal（备份 key + migrationState + 完成集，见 4.3）先行落定，1.4/2.7/4.2-4.7 依赖本 gate

## 1. 素材库数据层（assetStore / library）

- [ ] 1.1 AssetProject 类型 + `ingestProjectAsset`（vendor MIME 白名单）+ sys-projects seed 幂等；vitest：ingest/重名后缀/白名单拒绝
...
- [ ] 4.3 实验室模板面板库化：`lib/stores/templates.svelte.ts` 共享 record store + VariantEditor 数据源切换；CRUD（新建即 ingest/复制 fork/软删/重命名）；**保存=字段提交自动换绑**（[R1-E4] 每模板串行写队列 + 单调 revision，旧写不覆盖新写）；EffectRefControl 写回 caseBinding + 第四入口 [从素材库选]；**variants {v:2} 信封退役迁移（[R1-E3] create-only journal：一次性备份 key + migrationState + 逐节点完成集，全部成功才删旧 key；不做内容差异覆盖）**；applyTaskParams 非破坏化（表单回填+[复制提示词]）；vitest：CRUD 写路径/迁移幂等与中途失败重试/版本门 bump 后库模板零变化（主指标）/软删不复活
