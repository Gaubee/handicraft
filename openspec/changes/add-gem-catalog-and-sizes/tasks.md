...
1. [2026-09-19 Contract] [R2 终审 W0-GATE-ONLY 授权范围，与 design §0.1/§1.7 一致]：
   ...
   R2 P0-1 前两句 + P0-2 全文），行为测试半句在 1.7/3.2 最终合流门逐字引用收口。
...
- [ ] 1.7 **engine gate 验收（出处标注）**——R2 §四 P0-3（engine 部分）：「engine gate 完成逐钻 spec、pairwise/exportGate……多层导出包含隐藏层且违规硬阻断；BOM 使用 `specKey×colorId`」（多层导出接线归 studio-layers，判据/门在本切片证明）；R2 §一 P0-2：「统一签名、单位和 `maxCellPx` 公式；实现全层 concat 的 pairwise、保存 warning/导出 hard block；补大小径、跨 cell、边界 gap、旋转、20k 测试」；**R2 §四 P0-1 行为半句（R1 P0-1 移入）：「跨层 mixed-size/旋转/边界测试全部通过」在本切片收口**（单层行为测试 + 判据证明；跨层 concat 组织与 replay 联合验收归 replay/handoff gate）；gpu-research §7 P0-1：43s→<10s 零输出变化
...
- [ ] 3.2 Codex 评审 → 修订 → 归档候选；**最终合流门（R1 P0-1）：逐字引用 R2 §四 P0-1 全文收口**——「验收：TypeScript 编译、全库 grep 只有 canonical 名称、跨层 mixed-size/旋转/边界测试全部通过」（前两句 W0 receipt、第三句 engine gate 1.7 + replay 联合），与 R2 §四 P0-2 验收原文、gemspec R1 放行条件 1 一并逐条复核 + 消费面矩阵（studio-layers §E.6）中本 change 责任行的测试符号核对
