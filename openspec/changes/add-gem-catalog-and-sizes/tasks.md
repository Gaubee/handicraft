...
- [ ] 0.6 目录资产 schema 冻结 + 迁移 bootstrap 表（Owner 2026-09-20 裁决一）：`.gemshape` schema 增 `vectorPath?`（归一化 SVG path，单位框 0..1，与 texture 至少其一、并存导出优先 vectorPath）/`specKey?`（canonical 身份键——seed 资产必填如 round-ss10，自定义 ingest 派生 custom-<assetId>）；seed 数据形状（形 id/短码/中文名/贴图或 vectorPath/物理宽高）+ P0 五形（round/square/drop/heart/marquise）+ 身份不可变纪律（specKey 创建后不可变、seed 只读、改物理=另存副本）；engine 迁移 bootstrap 表（SS_TABLE 直径查表 + specKey 生成规则 round-ss10/square-3.5——非目录真源）；vitest：specKey 确定性 + path 归一化（单位框）+ seed 幂等 create-only（`ast-shape-${specKey}` 节点存在即跳过含软删）
- [ ] 0.7 **W0 contract receipt（R1 P0-1 修订：只验收契约面）**——① TypeScript 编译 + canonical 名称 grep（`specId` 身份字段/持久化别名清零；`refSpecId` 为独立校准引用字段、单独定义不计入禁令）；② helper 签名冻结（`requiredCenterDistancePx(a,b,grid)` 三参唯一 + `maxCellPx`，无二参/别名并存）；③ 四格式 v1 fixture 迁移演练 + v2 byte-round-trip + 向前拒读 + 脏输入 typed error；④ `.gemshape` 六条 gate 坏输入用例；⑤ PhysicalCanvas px/mm 不变量（**仅 schema/纯函数层，运行时接线不在 W0 验收面**）；⑥ serializer 首写证明：2.x 首个写入路径仅在 W0 receipt 通过后出现且不存在 v1 新写路径（写路径 grep）。R2 §四 P0-1 原文中「跨层 mixed-size/旋转/边界测试全部通过」依赖 engine/replay 实现——**移至 1.7 与最终合流门（3.2）收口，W0 不验收行为测试**；P0-2 验收原文（byte-round-trip/向前拒读/损坏·超限·悬空 ref 拒绝/missing 不得静默导出）与 gemspec R1 放行条件 1（「不接受只扩字段、不扩 `GridSpec`/handoff 的切片」）整条属本 receipt
