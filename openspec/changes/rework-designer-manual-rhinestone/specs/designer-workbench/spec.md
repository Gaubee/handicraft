...
### Requirement: 智能排布工具
智能排布 UI 入口 MUST 全部移除（顶栏按钮/右键菜单项/命令/面板组件）；`smartLayoutGemsFromImage` 钻数组内核与编排模块 MUST 零改动保留（归 add-designer-selection-paths 复用）。本版本设计师工作台聚焦手动排钻。

#### Scenario: 入口退役
- **WHEN** 打开设计师工作台任意文档
- **THEN** 顶栏与右键菜单均无智能排布入口；内核模块 diff 为零
