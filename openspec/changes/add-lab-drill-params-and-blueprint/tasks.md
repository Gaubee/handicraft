...
4. [2026-09-20 Lifecycle] LabTask 升级 stage 子状态（各自 requestId/status/assetId/error/retryCount，
   ...
   归档条件 = main success 且蓝图终态，双图一并定稿，重试产新档。
...
- [ ] 0.3 生命周期与蓝图契约冻结：`LabStage{id,kind:'main'|'blueprint',status,dependsOn,requestId,assetId,imageUrl,imageStored,error,debug,retryCount,时间戳}` + `StageEvent`/`reduceStages`/`deriveTaskStatus`/`schedulableStages` 签名（design §3.2）；父任务派生表逐行冻结（design §3.3）；操作粒度（cancelTask/cancelStage/retryStage/retryTask+级联失效 invalidate）；归档时点与部分失败语义（双图一并定稿、provenance.blueprint.status、重试产新档）；gemgen v2 消费口径（blueprint 键 + provenance.requestMode/gemSpecs/physicalCanvas/blueprint 快照 + blueprintPrompt 全文快照落 provenance）；vitest：契约类型编译 + 派生表枚举断言
- [ ] 0.4 契约 receipt：① TypeScript 编译；② grep 证明——新代码无 `workflowMode` 写路径、`specId` 身份字段清零（沿 add-gem-catalog W0 禁令，refSpecId 除外）；③ 0.1-0.3 的 vitest 全绿；④ 跨 change 同步核对——add-gem-catalog design §1.3 已按正交键修订（未修订则 4.x 冻结不开工，轨道 A/B/C 不受影响）
...
- [ ] 2.3 持久化合成：`PersistedTaskMeta.stages` 序列化/反序列化 + legacy 账本（无 stages）读时合成单 main stage 只读兼容；vitest：round-trip + legacy 合成 + 配额降级（剥 debug 时 stage 字段保全）
...
- [ ] 4.4 蓝图两策略与归档：策略 B 串行依赖（main success→edits 附图 [成品,原图,...素材,...蓝图参考] + composeBlueprintPrompt）+ 策略 A 并行派发；归档升级（archiveGeneratedResult：blueprint 键+provenance 四态、双图一并定稿、补偿链 blueprint 字节回取、归档后蓝图重试产新档）；任务卡数据源接线（消费 3.3）；vitest：策略 B 附图序端到端/策略 A 并发/蓝图单独重试/归档四态+missing custom asset 阻断
