...
- [ ] P1.1 参数化几何族扩展：星射线/心/圆/矩/椭圆/螺旋（参数 schema 冻结+预览确定性测试）
- [ ] P1.2 语义拟合族 v1：纹理亮暗（scatter/flow）+ 柔和曲线（骨架线）+ 花形（极坐标分解）——各≥1 真实 fixture
- [ ] P1.3 排除族：drillWorthy 开关贯穿（排布跳过+BOM 未贴区明示）
- [ ] P1.4 **自由代码族（最高优先）**：JS worker 沙箱（注入面=mask/标度/钻规格/几何库/Rand(seed)；无网无 fs；CPU/时有界）+ 输出 Zod 校验+强制引擎校验门+非法输出有界重试回 LLM+同 seed 回放——沙箱逃逸面审查（评审重点）
- [ ] P1.5 策略注册表：四类统一接口 `apply(node, params|code, canvas) → Gem[]`（引擎五策略降级为基础族成员）

## P2 识图管线（macmini 桥）

- [ ] P2.1 macmini 侧服务：spike.py 底座改常驻（stdin/stdout JSON 协议）+ SAM3 几何/语义双提示面 + 版本上报 + 启停脚本
- [ ] P2.2 daemon `sam-bridge.ts`：SSH 长连接+队列（并发 1）+120s 界+产物回传 BlobStore+输出留存目录（叠加图+meta）——mock 桥单测
