...
- [ ] 9.2b 〔Owner 指令 2026-09-21〕**vision 子代理真实走查门（Codex 终审前置，硬门）**：全部实现切片+9.1 绿门完成后、Codex 终审之前——主会话起**独立端口** dev server（严禁触碰 Owner 的 5200）+ 种子 fixtures；vision 子代理经真实浏览器全流程走查（设计师四区/笔刷规格/图层操作/导出链/右键与键位/空态三入口；实验室六点反馈面）+ **使用体验反馈报告**（coherence/journey/IA/state 四维 + 问题清单逐条可复现）；问题回修迭代后**重新走查，直至 vision 验收通过**方可进入 Codex 终审；走查截图先过程序化非平凡校验（黑图防线）；server 常驻进程由主会话持有并显式回收
- [ ] 9.3 真实浏览器验收（非阻塞⑤——jsdom 只证决策核，布局/事件接线以真浏览器走查收据为准）：Owner 重启本地 5200 dev 服务后走查四项——① pointer capture（P5 拖移/P6 旋转/P7 改径会话跨元素不丢焦、Esc 取消）② 原生 contextmenu（P13/P14 两态树定位与命令执行）③ 图层面板拖排（P16 z 序即时生效、SVG/PNG 合成序随之、BOM 行序不变）④ 移动端断点（§1.4 底部工具条/抽屉/双指手势）；走查偏离项登记回报（不静默修）
