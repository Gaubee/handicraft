...
- **字节级护栏（engine 面不动）**：`git diff --stat src/lib/engine` 零行（本 change 全程）；engine 族测试零改动；**persistence/services 护栏改为例外开窗清单口径**——仅 projectFile.ts（R1-P0-1）与 documentService.ts（R1-P0-2）允许 diff，且限于声明改动；quickLayout 同参同出快照（计算内核不动收据）；documentService/gemdocLifecycle 既有测试零断言改动（投影/迁移新增断言独立成新测试文件；守卫三分法行为等价）。〔窗口随记 2026-09-22，终审交接发现〕R0 术语扫描按 0.1 收据口径改写 engine 三处**注释**（edit.ts/index.ts/types.ts「专家工作台→设计师工作台」，各 1 行）——行为零变更（终审核验「未见引擎行为变更」）；此为术语全域 grep 要求所致，计入开窗随记，2.x 基线（1613fdf）起 engine 零行。〔窗口随记 2026-09-21，6/7 批裁决〕① gemdocLifecycle.svelte.ts `serializeCurrentGemdoc` painting 源改条件落位入窗（幻影行修复 A——伪造点在装配层，projectFile 只透传零 diff；v2 fixtures 零改动+字节等价性质保持）；② quickLayout.ts +124 行为 7.1 API 面新增（`smartLayoutGemsFromImage` 钻数组模式），内核/冻结参数/进度取消零改动，跨模式冻结等价四参下逐位相等测试锁内核零漂移。
