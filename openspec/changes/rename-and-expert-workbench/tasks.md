...
- [x] 5.6 gemCatalogService 真源切换【硬前置：2.x sys-shapes seed 落地】：mock → sys-shapes .gemshape 资产（含内置形入库 seed——入库口径 design §0.4）；engine 迁移 bootstrap 查表仅兜旧档；vitest：切换点等价（同 specKey 解析一致）+ mock 退役为测试夹具
  〔2026-09-20 核验勾选：真源切换已由 add-lab 4.2 执行（gemCatalogService.ts:7-14 头注「[add-lab 4.2] 切换已执行」+ :47-48「[add-lab 4.2] 执行 5.6 真源切换点·接口签名不变」登记）——生产单例 gemCatalog = createLibraryGemCatalogService()（sys-shapes .gemshape 真源），createInMemoryGemCatalogService 退役为 vitest 夹具（生产代码零消费）。收据复跑：services.acceptance（5.6 marker 四断言）+ gemCatalog（mock 夹具 5 例）+ gemCatalog.library（真源 listSpecs 声明序 20 条/resolveSpec 四态/自定义 custom-<assetId> 携带 assetId）4 文件 24 例全绿；R4 评审时 marker 漂移致 85/86 已消（当前树 gemCatalogService.ts 含 5.6/sys-shapes/.gemshape/接口签名不变四标记）〕
- [x] 5.7 画幅物理读数接线【硬前置：studio-layers replay/handoff gate 贯通 PhysicalCanvas】：状态条读数位接真值（画幅 mm + px/mm；anchorSource 区分 declared/default 显示）；vitest：declared/default 两态读数 + 降采样锚不变量（消费端呈现）
  〔2026-09-20 完成：硬前置已由 studio-layers ③段贯通（EditDocument.physicalCanvas 必填——a3e59ed 装载后恒有）；EditView.svelte 状态条 canvas={doc?.physicalCanvas ?? null} 接真值（null 兜底保持「未锚定」占位——缺真源不显示假值）；EditStatusBar anchorSource 区分（declared 直读 / default 标「缺省锚」）。vitest workbench.physicalReadout 4 例：declared 读数 210×148mm+2.5px/mm 无缺省锚标注 / default（v1 载荷无键合成 25.6mm）显式缺省锚 / 降采样锚不变量（pixelsPerMmFromCanvas(doc.width, canvas)=grid.pixelsPerMm）消费端呈现 / null 兜底未锚定〕
- [ ] 5.8 D 轨验收：上述全部 vitest 面 + 消费端到端（选 spec → 笔刷落钻 → 属性面板改径 → 警告 → 保存放行/导出阻断）jsdom 走查
- [x] 5.9 handoff/gemdoc payload v2 消费接线【硬前置：studio-layers replay/handoff gate 验收完成（③ 段 gate；非本仓编号）——R3 P0 修复新增切片】：buildManualEditHandoff v2 payload 消费（各层 concat、逐钻 GemSpecSnapshot、PhysicalCanvas——按 gate 冻结契约）/ loadFromHandoff v2 消费 / EditDocument·gemdoc schema 与 round-trip 接线；落点 = A 轨 2.3/2.4 搬移出的 `editHandoff.svelte.ts`/`gemdocLifecycle.svelte.ts`（payload 首个修改点，此前 A 轨只维护薄 wrapper）；vitest：逐层 handoff identity + round-trip（沿 gate 验收口径）+ 无双写路径断言（rg/import 面：payload 构造唯一入口）
  〔2026-09-20 核验勾选：v2 消费已由 studio-layers ③段 1.4 完成并接管修改权（a3e59ed——expert design §2.2 交接表落点即本行指定两文件：editHandoff.svelte.ts buildManualEditHandoff 换真「各层 concat 逐钻物化规格+default 锚显式+sourceSummary 层语法」、gemdocLifecycle.svelte.ts loadFromHandoff/serializeCurrentGemdoc/loadFromGemdoc 三点接线 + 旧档无键 default 补齐；EditDocument.physicalCanvas 必填）。收据复跑：handoffV2 8 例（v2 换真/gems 层 concat/physicalCanvas default 显式/sourceSummary 层语法/gemdoc round-trip declared·default 两态字节等价）全绿——studio-layers tasks 1.4 勾选在档（3e4c5c5 验收 receipt 同链）〕
