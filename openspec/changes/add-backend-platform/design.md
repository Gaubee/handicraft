...
| 密钥 | .env 模板自建/0600/原位回写 + settings 表优先双层真源 + 半配置=未配置 | 同；键族=图像 API（IMG_BASE_URL/IMG_API_KEY/IMG_MODEL）+ Agent LLM（LLM_* 同 zhumo）+ JWT_SECRET/ADMIN_*/DATA_ROOT。**读面一律脱敏（键存在性+尾 4 位），写入口仅 .env 与 settings RPC（写入也脱敏回显）**；**持久面脱敏不变量（R8 冻结，R10 形态集枚举完整化）=「合规密钥（trim≥8 且不含 *，配置侧校验拒病态形态）的原文及其 base64/percent/hex 三种命名编码的全部标准表示变体（base64=四组合枚举{standard/base64url}×{padded/无 padding}；percent/hex=解码等价类正则匹配——percent 逐字节「原文|%HH」可选转义且 hex 位大小写无关、hex 逐 nibble 大小写无关，覆盖全部标准表示组合）不出现在任何持久化 debug/error 面」——任意可逆编码/哈希/碎片形态在信息论上无界，超出本地单用户 daemon 威胁模型（终门=deepReplaceSecret 键值双面替换+maskEncodings）BYOK localStorage 路径随实验室旗标退场，Agent 主面不依赖浏览器本地密钥 |
