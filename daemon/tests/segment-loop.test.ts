 ...
 * P0.2 tree-to-blocks。零常驻进程：全同步 mock，无定时器无连接。
 */
import { describe, expect, it } from 'vitest';
import type { NodeBBox, SceneElement } from '@handicraft/contracts';
import {
...
/** 80×80px ↔ 8×8cm ⇒ pixelsPerMm=1（mm 断言免换算）。 */
const GEOM = { taskId: 'task-seg', imageBlobRef: 'a'.repeat(64), imagePx: { width: 80, height: 80 }, canvasCm: { w: 8, h: 8 } } as const;

/** 判据 1 阈值=K2.5×2mm=5mm=5px（ppm=1）；measure 高方差（22>10——色判据不触发）。 */
...
function segOutcome(box: NodeBBox | null, score?: number): SegmentBridgeOutcome {
  return {
    mask: { w: 80, h: 80, bits: box === null ? new Uint8Array(80 * 80) : rect(80, 80, box) },
...
/** 三轮收敛脚本：A 路灯+B 草地（首轮各 30×30）→次轮 A 裂 5×5 子+B 空掩码→三轮 A 空掩码。 */
function convergenceFixture() {
  const elements = [
    element('路灯', 'streetlight', { x: 0, y: 0, w: 30, h: 30 }),
    element('草地', 'grassland', { x: 40, y: 40, w: 30, h: 30 }),
  ];
  const script = new ScriptedSegment([
    () => segOutcome({ x: 0, y: 0, w: 30, h: 30 }),
    () => segOutcome({ x: 40, y: 40, w: 30, h: 30 }),
    () => segOutcome({ x: 0, y: 0, w: 5, h: 5 }), // A 次轮子（5mm=阈值上——判据 1 封停）
...
describe('三轮收敛（首轮 2 元素→次轮 1 裂子→三轮全停）', () => {
  it('树结构：画布根+2 元素+A 的 1 子；封停原因 size/model 各就位；请求序几何×2→语义×3', async () => {
    ...
    expect(script.requests[0]!.prompt).toEqual({
      kind: 'geometric',
      points: [{ x: 15, y: 15, label: 'include' }],
      box: { x: 0, y: 0, w: 30, h: 30 },
    });
    expect(script.requests[1]!.prompt).toEqual({
      kind: 'geometric',
      points: [{ x: 55, y: 55, label: 'include' }],
      box: { x: 40, y: 40, w: 30, h: 30 },
    });
    ...
    // 节点字段：bbox=掩码紧外接；effectiveMm=box 换算（ppm=1）；depth 语义（首轮=1）
    expect(lamp.bbox).toEqual({ x: 0, y: 0, w: 30, h: 30 });
    expect(part.bbox).toEqual({ x: 0, y: 0, w: 5, h: 5 });
    expect(lamp.effectiveMm).toBe(30);
    expect(part.effectiveMm).toBe(5);
    expect(lamp.mask).toMatchObject({ kind: 'inline', w: 30, h: 30 });
...
describe('根裁定（design §1 S5：多主体=画布根/单主体=该主体）', () => {
  it('单主体：根=该元素自身（无画布节点），后续子挂其下', async () => {
    const script = new ScriptedSegment([
      () => segOutcome({ x: 10, y: 10, w: 40, h: 40 }),
      () => segOutcome({ x: 10, y: 10, w: 8, h: 8 }), // 子（8mm>5 不停）
      () => segOutcome(null), // 子轮零实例→子封；父仍活
      () => segOutcome(null), // 父零实例→全停
    ]);
    const result = await runSegmentLoop(
      { ...BASE, elements: [element('人物', 'person', { x: 10, y: 10, w: 40, h: 40 })] },
      depsOf(script),
    );
  ...
  it('多主体：画布根=全画布 mask、effectiveMm=整幅换算、labVariance=整幅测量', async () => {
    ...
    expect(canvas.bbox).toEqual({ x: 0, y: 0, w: 80, h: 80 });
    expect(canvas.effectiveMm).toBe(80); // √(80·80)/1
    expect(measured).toContain('80x80'); // 整幅测量入列（元素×2+子×1+画布=4 次）
...
describe('停止判据内嵌（P0.3 evaluateStopCriteria 逐条触发）', () => {
  it('判据 1 尺寸：元素 4×4px（4mm≤阈值 5mm）出生即封停，一轮即终态', async () => {
    const script = new ScriptedSegment([() => segOutcome({ x: 0, y: 0, w: 4, h: 4 })]);
    const result = await runSegmentLoop(
      { ...BASE, elements: [element('小花', 'small flower', { x: 0, y: 0, w: 6, h: 6 })] },
      depsOf(script),
    );
  ...
  it('判据 2 色差：measure 返回 ΔE=3（≤FAMILY 10，天空/草地类平色）出生即封停', async () => {
    ...
    const result = await runSegmentLoop(
      { ...BASE, elements: [element('天空', 'sky', { x: 0, y: 0, w: 40, h: 40 })] },
  ...
  it('判据 3 模型自认 no-new-instance：细分零交集→父封停（主停止器——§9 实测口径）', async () => {
    ...
    const result = await runSegmentLoop(
      { ...BASE, elements: [element('房子', 'house', { x: 0, y: 0, w: 30, h: 30 })] },
      depsOf(script),
    );
  ...
  it(`判据 3 模型自认 low-score：score<${SEGMENT_LOOP_LOW_SCORE} 弱实例不入树+父封停`, async () => {
    const script = new ScriptedSegment([
      () => segOutcome({ x: 0, y: 0, w: 30, h: 30 }),
      () => segOutcome({ x: 0, y: 0, w: 10, h: 10 }, 0.3), // 有效交集但弱实例
    ]);
    const result = await runSegmentLoop(
      { ...BASE, elements: [element('马车', 'carriage', { x: 0, y: 0, w: 30, h: 30 })] },
      depsOf(script),
    );
  ...
  it('边界：score=0.5 不算弱实例（< 闭合界）——正常建子', async () => {
    const script = new ScriptedSegment([
      () => segOutcome({ x: 0, y: 0, w: 30, h: 30 }),
      () => segOutcome({ x: 0, y: 0, w: 10, h: 10 }, SEGMENT_LOOP_LOW_SCORE),
      () => segOutcome(null),
      () => segOutcome(null),
    ]);
    const result = await runSegmentLoop(
...
describe('mask 交集防外溢（父∩子位与——§9 回流 3）', () => {
  it('子响应越出父域 5 列：位与裁掉——子 bbox 收紧到交集紧外接', async () => {
    const script = new ScriptedSegment([
      () => segOutcome({ x: 0, y: 0, w: 20, h: 20 }), // 父（画布左上 20×20）
      () => segOutcome({ x: 10, y: 0, w: 15, h: 15 }), // 子响应 x∈[10,25)——越父右界 x=19 共 5 列
      () => segOutcome(null), // 子轮 2 零实例→子封
      () => segOutcome(null), // 父轮 2 零实例→父封
    ]);
    const result = await runSegmentLoop(
      { ...BASE, elements: [element('树叶', 'leaf', { x: 0, y: 0, w: 20, h: 20 })] },
      depsOf(script),
    );
    const child = result.tree.nodes[1]!;
    expect(child.objectName).toBe('树叶·部分1');
    expect(child.bbox).toEqual({ x: 10, y: 0, w: 10, h: 15 }); // w=10 非 15——越界列裁掉
    expect(child.effectiveMm).toBe(Math.sqrt(150));
    // 子 mask=bbox 局部同维（tree-to-blocks 直喂不变式）且全 1（实心交集）
    expect(child.mask).toMatchObject({ kind: 'inline', w: 10, h: 15 });
  });
});
...
describe('vlmReentry 接口位（design §2——默认 false）', () => {
  ...
  it('true：走桥 analyze 重新描述——精化 hint 进细分提示（接口接线，VLM 真连归 P2.6）', async () => {
    const script = new ScriptedSegment([
      () => segOutcome({ x: 0, y: 0, w: 30, h: 30 }), // A 首轮
      () => segOutcome({ x: 0, y: 0, w: 30, h: 30 }), // B 首轮
      () => segOutcome({ x: 0, y: 0, w: 6, h: 6 }), // A 轮 1 裂子（6mm>5 阈→子继续）
    ...
    const analyze = new ScriptedAnalyze([
      () => ({
        elements: [element('路灯灯头', 'streetlight lamp head', { x: 0, y: 0, w: 10, h: 10 })],
      }),
  ...
  it('true 但未注入 analyze：init 即 typed 拒（接口位缺位不静默降级）', async () => {
    const script = new ScriptedSegment([() => segOutcome({ x: 0, y: 0, w: 30, h: 30 })]);
    const error = (await runSegmentLoop(
      { ...BASE, elements: [element('路灯', 'streetlight', { x: 0, y: 0, w: 30, h: 30 })], vlmReentry: true },
      depsOf(script),
...
describe('首轮提示面（几何=box / 能力降级=hint 文本）', () => {
  it("elementPromptMode='hint'：首轮用元素英文 hint 文本提示（桥几何面不可用降级）", async () => {
    ...
    await runSegmentLoop(
      { ...BASE, elementPromptMode: 'hint', elements: [element('路灯', 'streetlight', { x: 0, y: 0, w: 30, h: 30 })] },
      depsOf(script),
    );
    expect(script.requests[0]!.prompt).toEqual({ kind: 'text', text: 'streetlight' });
  });

  it('元素类别缺省 subject；suggestDrillWorthy=false 透传（排除开关面）', async () => {
    const script = new ScriptedSegment([
      () => segOutcome({ x: 0, y: 0, w: 30, h: 30 }),
      () => segOutcome(null),
    ]);
    const result = await runSegmentLoop(
      { ...BASE, elements: [element('黑背景', 'black background', { x: 0, y: 0, w: 30, h: 30 }, { suggestDrillWorthy: false, category: undefined })] },
      depsOf(script),
    );
    const node = result.tree.nodes[0]!;
    expect(node.category).toBe('subject'); // P4.1 词表版本化前的中性兜底
...
describe('失败面与坏输入（typed，不静默吞）', () => {
  ...
  it('桥 segment 抛错：bridge-failure typed（cause 保留）——降级归 P2.5 裁量', async () => {
    ...
    const error = await capture(
      runSegmentLoop({ ...BASE, elements: [element('路灯', 'streetlight', { x: 0, y: 0, w: 30, h: 30 })] }, depsOf(script)),
    );
    expect(error).toBeInstanceOf(SegmentLoopError);
  ...
  it('桥掩码维度≠画布：bad-mask typed（全图坐标锚点——不猜裁剪）', async () => {
    ...
    expect(error.message).toMatch(/≠ 画布 80×80/);
  });

  it('桥掩码字节非法（值 2）：bad-mask typed', async () => {
    const bits = new Uint8Array(80 * 80).fill(2);
    const script = new ScriptedSegment([() => ({ mask: { w: 80, h: 80, bits } })]);
    const error = await capture(
      runSegmentLoop({ ...BASE, elements: [element('路灯', 'streetlight', { x: 0, y: 0, w: 30, h: 30 })] }, depsOf(script)),
    );
    expect(error.kind).toBe('bad-mask');
      ...
      runSegmentLoop(
        {
          ...
          elements: [element('路灯', 'streetlight', { x: 0, y: 0, w: 30, h: 30 })],
        },
        depsOf(script),
      ),
    );
  ...
  it('坏参数：元素空清单/非正 maxGemDiameterMm/非正 maxNodes → bad-option typed', async () => {
    const script = new ScriptedSegment([]);
    const elements = [element('路灯', 'streetlight', { x: 0, y: 0, w: 30, h: 30 })];
    for (const over of [
...
describe('确定性（同 seed 同请求序同产物——回放纪律）', () => {
  it('同脚本双跑：请求序逐字段相等+终树/封停表/演化 meta 深比较相等', async () => {
    const run = () => {
      const { elements, script } = convergenceFixture();
...
describe('产出树直喂 P0.2 tree-to-blocks（循环产出的树要能喂它）', () => {
  it('inline mask 免 blob 直读：块=非容器节点全列、blockId=节点 id、mask/bbox 同维', async () => {
    ...
    expect(outcome.pixelsPerMm).toBe(1);
    ...
    expect(lampBlock?.origin.nodeCategory).toBe('subject');
