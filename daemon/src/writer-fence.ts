 ...
 * 同一「session/task 仍可写」判定——fence 于 session.status 与 task 行存在性/归属。
...
/**
 * fence 判定（§6.5 R3）：task 行存在，且（若属会话）session.status==='active'。
 * 与 JobService.emitFrame 的帧 fence 同谓词——帧/产物/blob 三类 writer 一致。
 */
